...
| `EditorToolbar.tsx` | Compact identity + actions row: filename, layer, conformance chip, `↶`/`↷`, Discard/Save, `{ history }`/`{ raw }`/`⤢`. The old recipe header's path/size/modified fields moved into `{ raw }`. |
