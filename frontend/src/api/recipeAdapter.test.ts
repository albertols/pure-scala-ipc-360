...
describe('recipeToCanvas — nodes, kinds, ports', () => {
  ...
  it('tolerates the pre-repair weststone key (defensive)', () => {
    const damaged = JSON.parse(
      JSON.stringify(bizlink).replaceAll('"fields":', '"weststone":'),
    ) as RecipeJson
  ...
  it('kind map + fixed labels for union/normalizer/java/storedProcedure/intermediate-table', () => {
    const mk = (type: string): RecipeJson => ({
      steps: [
        { target: { name: 'X', type, fields: [] }, sources: [] },
    ...
    const sq = recipeToCanvas(mk('sourceQualifier'), 'L/x/_ETL_x.json').nodes.find(
      x => x.id === 'X',
    )!
    expect(sq.type).toBe('sq')
  })
  it(
    'typeAliases (from GET /api/ipc/rules) resolve the four anonymizer tokens to their canonical kind, ' +
    ...
    () => {
      ...
      for (const [aliasType, canonicalType, expectType, expectLabel] of cases) {
        const aliased = recipeToCanvas(mk(aliasType), 'L/x/_ETL_x.json', typeAliases).nodes.find(
          x => x.id === 'X',
        )!
        const canonical = recipeToCanvas(
          mk(canonicalType),
          'L/x/_ETL_x.json',
          typeAliases,
        ).nodes.find(x => x.id === 'X')!
  ...
  it('typeAliases defaults to {} — an aliased token with no aliases supplied still falls back (backward compat)', () => {
    const r: RecipeJson = {
      steps: [
        { target: { name: 'X', type: 'BERYLFALLS', fields: [] }, sources: [] },
        { target: { name: 'T', type: 'table', fields: [] }, sources: [] },
      ],
      table: { targetTableNames: ['T'], sourceTableNames: [] },
    }
  ...
  describe('declared source tables', () => {
    ...
    it('the very first insertion into an empty draft is visible (the from-scratch case)', () => {
      const r: RecipeJson = {
        steps: [],
        table: { targetTableNames: [], sourceTableNames: ['CAS_ODS_EVENTS'] },
      }
        ...
        steps: [
          {
            target: {
              ...
              type: 'table',
              fields: [
                { name: 'A', dataType: 'String', transformation: { source: 'LKP_REFD.CODE' } },
              ],
            },
            sources: [],
          },
    ...
    it('a blank name in sourceTableNames is ignored, not rendered as an empty node', () => {
      const r: RecipeJson = {
        steps: [],
        table: { targetTableNames: [], sourceTableNames: ['', 'OK'] },
      }
  ...
  const unionRecipe: RecipeJson = {
    steps: [
      {
        target: {
          name: 'DWH_E_LKP_DIR_PHONELIST',
          type: 'table',
          fields: [
            {
              name: 'ID_LOCATION',
              dataType: 'BigDecimal',
              transformation: { source: 'Union.ID_LOCATION' },
            },
            {
              name: 'ID_MEMBER',
              dataType: 'BigDecimal',
              transformation: { source: 'Union.ID_MEMBER' },
            },
          ],
        },
        sources: [
      ...
      {
        target: { name: 'MAPLEROAD301MAPLEHEATH', type: 'EARLYGLADE', fields: [] },
        sources: [{ name: 'SQ_X1', type: 'BERYLFALLS' }],
      },
      {
        target: { name: 'MAPLEROAD100MAPLEHEATH', type: 'EARLYGLADE', fields: [] },
        sources: [{ name: 'SQ_X2', type: 'BERYLFALLS' }],
      },
      {
        target: { name: 'SQ_X1', type: 'BERYLFALLS', fields: [] },
        sources: [{ name: 'T_X1', type: 'table' }],
      },
      {
        target: { name: 'SQ_X2', type: 'BERYLFALLS', fields: [] },
        sources: [{ name: 'T_X2', type: 'table' }],
      },
    ],
  ...
  const joinerRecipe: RecipeJson = {
    steps: [
      {
        target: {
          name: 'DWH_MAPLEGROVE_ACT_CLIENTMGR_PROFILES',
          type: 'table',
          fields: [
            {
              name: 'ID_MEMBER',
              dataType: 'Long',
              transformation: { source: 'JNR_Ashshore.ID_MEMBER' },
            },
          ],
        },
        sources: [
      ...
      {
        target: { name: 'JNR_Ashshore.DETAIL', type: 'ASHPATH2', fields: [] },
        sources: [{ name: 'SQ_D', type: 'BERYLFALLS' }],
      },
      {
        target: { name: 'JNR_Ashshore.MASTER', type: 'ASHPATH2', fields: [] },
        sources: [{ name: 'SQ_M', type: 'BERYLFALLS' }],
      },
      {
        target: { name: 'SQ_D', type: 'BERYLFALLS', fields: [] },
        sources: [{ name: 'D_TBL', type: 'table' }],
      },
      {
        target: { name: 'SQ_M', type: 'BERYLFALLS', fields: [] },
        sources: [{ name: 'M_TBL', type: 'table' }],
      },
    ],
    table: {
      targetTableNames: ['DWH_MAPLEGROVE_ACT_CLIENTMGR_PROFILES'],
      sourceTableNames: ['D_TBL', 'M_TBL'],
    },
  }
  ...
  it('each unionInput step gets an edge to the union node it belongs to', () => {
    const g = recipeToCanvas(unionRecipe, 'DWH/x/_ETL_x.json', unionTypeAliases)
    expect(g.connections).toContainEqual({
      fromNode: 'MAPLEROAD301MAPLEHEATH',
      fromPort: '',
      toNode: 'Union',
      toPort: '',
    })
    expect(g.connections).toContainEqual({
      fromNode: 'MAPLEROAD100MAPLEHEATH',
      fromPort: '',
      toNode: 'Union',
      toPort: '',
    })
    ...
    expect(g.connections).toContainEqual({
      fromNode: 'Union',
      fromPort: 'ID_LOCATION',
      toNode: 'DWH_E_LKP_DIR_PHONELIST',
      toPort: 'ID_LOCATION',
    })
  })
  ...
  it('joiner source becomes a canvas node: type joiner, label JNR, joinerType/joinerCondition lifted to properties', () => {
    ...
    expect(jnr!.ports.map(p => p.name).sort()).toEqual([
      'JNR_Ashshore.DETAIL',
      'JNR_Ashshore.MASTER',
    ])
  ...
  it('each joinerInput step (<joiner>.<MASTER|DETAIL>) gets an edge to the joiner named by the segment before the trailing dot', () => {
    const g = recipeToCanvas(joinerRecipe, 'DWH/x/_ETL_x.json', joinerTypeAliases)
    expect(g.connections).toContainEqual({
      fromNode: 'JNR_Ashshore.DETAIL',
      fromPort: '',
      toNode: 'JNR_Ashshore',
      toPort: '',
    })
    expect(g.connections).toContainEqual({
      fromNode: 'JNR_Ashshore.MASTER',
      fromPort: '',
      toNode: 'JNR_Ashshore',
      toPort: '',
    })
    expect(g.connections).toContainEqual({
      fromNode: 'JNR_Ashshore',
      fromPort: 'ID_MEMBER',
      toNode: 'DWH_MAPLEGROVE_ACT_CLIENTMGR_PROFILES',
      toPort: 'ID_MEMBER',
    })
  })

  it(
    'a joiner whose own name contains a dot: both joinerInput branches still resolve to the ' +
            ...
            sources: [
              {
                name: 'A.B',
                type: 'joiner',
                joinerTables: ['A.B.MASTER', 'A.B.DETAIL'],
              } as RecipeSourceJson,
            ],
          },
      ...
      steps: [
        {
          target: {
            name: 'T1',
            type: 'table',
            fields: [{ name: 'A', dataType: 'String', transformation: { source: 'Union.A' } }],
          },
          sources: [
            {
              name: 'Union',
              type: 'union',
              unionTables: [{ name: 'U1', fieldMapping: [{ origin: 'A1', union: 'A' }] }],
            } as RecipeSourceJson,
          ],
        },
        {
  ...
  it('an aliased union/joiner source type resolves identically to the canonical one (typeAliases threaded through, not hardcoded)', () => {
      ...
      steps: [
        {
          target: {
            name: 'T',
            type: 'table',
            fields: [{ name: 'A', dataType: 'String', transformation: { source: 'U.A' } }],
          },
          sources: [
            {
              name: 'U',
              type,
              unionTables: [{ name: 'IN1', fieldMapping: [{ origin: 'A1', union: 'A' }] }],
            } as RecipeSourceJson,
          ],
        },
      ],
    ...
    const aliased = recipeToCanvas(
      mkUnion('MY_UNION_ALIAS'),
      'L/x/_ETL_x.json',
      aliases,
    ).nodes.find(n => n.id === 'U')!
    const canonical = recipeToCanvas(mkUnion('union'), 'L/x/_ETL_x.json', aliases).nodes.find(
      n => n.id === 'U',
    )!
      ...
      steps: [
        {
          target: { name: 'T', type: 'table', fields: [] },
          sources: [
            { name: 'J', type, joinerTables: ['J.MASTER', 'J.DETAIL'] } as RecipeSourceJson,
          ],
        },
      ],
      table: { targetTableNames: ['T'], sourceTableNames: [] },
    })
    const aliasedJ = recipeToCanvas(
      mkJoiner('MY_JOINER_ALIAS'),
      'L/x/_ETL_x.json',
      aliases,
    ).nodes.find(n => n.id === 'J')!
    const canonicalJ = recipeToCanvas(mkJoiner('joiner'), 'L/x/_ETL_x.json', aliases).nodes.find(
      n => n.id === 'J',
    )!
...
describe('recipeToCanvas — edges, formulas, layout', () => {
  it('derives field edges from dot-refs, case-insensitive from-node resolution, deduped', () => {
    ...
    expect(g.connections).toContainEqual({
      fromNode: 'SQ_ff_BIZLINK',
      fromPort: 'GREENBLUFF',
      toNode: 'BIZLINK',
      toPort: 'GREENBLUFF',
    })
    expect(
      g.connections.some(c => c.fromNode === 'ff_BIZLINK' && c.toNode === 'SQ_ff_BIZLINK'),
    ).toBe(true) // FF_ -> ff_
  ...
  it('nested parameter walk yields edges from deep {source} refs (SYN lookup tree)', () => {
    const g = recipeToCanvas(syn as RecipeJson, 'ODS/m_SYN_ODS_ORDERS/_ETL_m_SYN_ODS_ORDERS.json')
    expect(g.connections).toContainEqual({
      fromNode: 'STG_L_SYN_ORDERS',
      fromPort: 'CURRENCY_CODE',
      toNode: 'ODS_SYN_ORDERS',
      toPort: 'AMOUNT',
    })
      ...
      steps: [
        {
          target: {
            name: 'T',
            type: 'table',
            fields: [{ name: 'A', dataType: 'String', transformation: { source: 'GHOST.A' } }],
          },
          sources: [],
        },
      ],
  ...
  it('ƒ rule + renderFormula: call trees render deterministically; plain source/value set no expression', () => {
    ...
    expect(
      s.nodes.find(n => n.id === 'ODS_SYN_ORDERS')!.ports.find(p => p.name === 'AMOUNT')!
        .expression,
    ).toBe(
