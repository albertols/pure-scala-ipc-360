...
const push = <K>(m: Map<K, string[]>, k: K, v: string) => {
  const a = m.get(k)
...
export function toDagClusters(rel: RelationshipsT): DagCluster[] {
  ...
  for (const r of recipes) {
    ...
    const a = byWorkflow.get(wf)
    a ? a.push(r) : byWorkflow.set(wf, [r])
  }
  ...
  return [...byWorkflow.entries()]
    ...
    .map(([wf, members]) => {
      ...
      return {
        dag_id: wf,
        schedule: `${tasks.length} recipe${tasks.length === 1 ? '' : 's'}`,
        last_run: '',
        status: 'skipped' as DagStatus,
        tasks,
      }
    })
}
...
export function overlayRun(cluster: DagCluster, rows: B15RowT[] | undefined): DagCluster {
  ...
  const status: DagStatus = set.has('failed')
    ? 'failed'
    : set.has('success') || set.has('running')
      ? 'success'
      : 'skipped'
  const ids = new Set(cluster.tasks.map(t => t.task_id))
  const last_run =
    (rows ?? [])
      .filter(r => ids.has(r.recipeFilename ?? ''))
      .map(r => r.appStartIso ?? '')
      .sort()
      .at(-1) ?? ''
...
export function clusterRuns(
  ...
  return [...dates].sort().map(date => {
    ...
    const status: DagStatus = statuses.has('failed')
      ? 'failed'
      : statuses.has('success') || statuses.has('running')
        ? 'success'
        : 'skipped'
    return {
      run_id: date,
      dag_id: cluster.dag_id,
      status,
      started_at:
        onDate
          .map(r => r.appStartIso ?? '')
...
const STATUS_UP: Record<DagStatus, StatusType> = {
  success: 'OK',
  failed: 'KO',
  running: 'RUNNING',
  skipped: 'PENDING',
...
export function toOperationalCard(
  task: DagTask,
  runs: RunT[],
  selectedDate: string,
): OperationalCard {
  ...
  const durations = oldestFirst
    .map(r => Math.round((r.durationMin ?? 0) * 60))
    .filter(n => n > 0)
    .sort((a, b) => a - b)
  const pct = (p: number) =>
    durations.length
      ? durations[
          Math.min(durations.length - 1, Math.max(0, Math.ceil((p / 100) * durations.length) - 1))
        ]
      : 0
  ...
  return {
    ...
    kind: 'recipe',
    name: task.task_id,
    layer: task.recipe_id.includes('/')
      ? task.recipe_id.slice(0, task.recipe_id.indexOf('/'))
      : '—',
