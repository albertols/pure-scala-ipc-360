...
import {
  ...
  buildLoggingUrl,
  buildDataprocJobUrl,
  buildDataprocClusterUrl,
  buildBigQueryUrl,
  DEFAULT_LOGGING_URL,
  DEFAULT_BIGQUERY_URL,
} from './gcpLinks'
...
describe('fillGcpUrl', () => {
  it('percent-encodes ordinary placeholders', () => {
    expect(fillGcpUrl('https://x/{a}?p={b}', 'unused', { a: 'v 1', b: 'w' })).toBe(
      'https://x/v%201?p=w',
    )
  })

  it('prefers the served template over the fallback', () => {
    expect(fillGcpUrl('https://served/{a}', 'https://fallback/{a}', { a: 'z' })).toBe(
      'https://served/z',
    )
    expect(fillGcpUrl(undefined, 'https://fallback/{a}', { a: 'z' })).toBe('https://fallback/z')
  })
  ...
  it('drops a matrix segment whose value is empty rather than emitting ";key="', () => {
    expect(
      fillGcpUrl(
        'https://x/q;cursorTimestamp={cursorTimestamp};duration={duration}?p=1',
        'unused',
        { cursorTimestamp: '', duration: 'P31D' },
      ),
    ).toBe('https://x/q;duration=P31D?p=1')
  })

  it('drops a trailing empty matrix segment before the query string', () => {
    expect(fillGcpUrl('https://x/q;duration={duration}?p=1', 'unused', { duration: '' })).toBe(
      'https://x/q?p=1',
    )
  })

  it('drops an empty matrix segment at the very end of the url', () => {
    expect(fillGcpUrl('https://x/q;duration={duration}', 'unused', { duration: '' })).toBe(
      'https://x/q',
    )
  })
})
...
describe('buildDataprocJobUrl / buildDataprocClusterUrl', () => {
  it('fill project and region from the served config', () => {
    expect(buildDataprocJobUrl(CFG, { jobId: 'j1' })).toBe(
      'https://console.cloud.google.com/dataproc/jobs/j1?project=example-project&region=europe-southwest1',
    )
    expect(buildDataprocClusterUrl(CFG, { clusterName: 'c1' })).toBe(
      'https://console.cloud.google.com/dataproc/clusters/c1?project=example-project&region=europe-southwest1',
    )
  })
})
...
describe('buildBigQueryUrl', () => {
  it('fills the project from the served config', () => {
    expect(buildBigQueryUrl(CFG)).toBe(
      'https://console.cloud.google.com/bigquery?project=example-project',
    )
  })
