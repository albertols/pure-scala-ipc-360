...
describe('toDagClusters — grouping, edges, layout', () => {
  it('groups recipe nodes by workflow, sorted; tables never become tasks', () => {
    ...
    expect(
      clusters.flatMap(c => c.tasks.map(t => t.task_id)).every(id => id.endsWith('.json')),
    ).toBe(true)
  ...
  it('layout: executionOrder ranks columns, intra-cluster deps bump right, ties stack rows', () => {
    ...
    expect(stg.tasks.map(t => [t.x, t.y])).toEqual([
      [60, 80],
      [60, 200],
    ]) // same order 1 -> one column, stacked by name
  ...
  it('no run overlay yet: tasks and clusters default to no-data (skipped/grey)', () => {
    for (const c of toDagClusters(REL)) {
      ...
      for (const t of c.tasks) {
        expect(t.last_status).toBe('skipped')
  ...
  it('cycle-safe: a dependency loop does not hang', () => {
    ...
    cyclic.edges!.push({
      from: 'table:FIX_ODS_B',
      to: 'recipe:_ETL_m_FIX_ODS_A.json',
      kind: 'source',
    })
  ...
  it('recipe nodes without a workflow fall into the UNGROUPED cluster', () => {
    const rel: RelationshipsT = {
      nodes: [
        {
          id: 'recipe:_ETL_m_LONER.json',
          kind: 'recipe',
          name: '_ETL_m_LONER.json',
          layer: 'STG',
          mappingPath: 'STG/m_LONER',
          hasRecipe: true,
          executionOrder: 1,
        },
      ],
...
const ROWS: Record<string, B15RowT[] | undefined> = {
  '2026-07-28': [
    row('_ETL_m_FIX_ODS_A.json', 'SUCCESS', '10m 00sec', {
      appStartIso: '2026-07-28T04:00:00.000Z',
    }),
  ],
  '2026-07-29': [
    row('_ETL_m_FIX_ODS_A.json', 'FAILED', '5m 30sec', { message: 'Stage failure (synthetic)' }),
  ],
}
...
describe('flow hardening — UNGROUPED, no-data recipes, degenerate inputs', () => {
  it('nodes with workflow "" or workflow absent both land in UNGROUPED, which sorts among clusters by name like any other', () => {
    ...
    rel.nodes!.push(
      {
        id: 'recipe:_ETL_m_FIX_EMPTYWF.json',
        kind: 'recipe',
        name: '_ETL_m_FIX_EMPTYWF.json',
        layer: 'STG',
        mappingPath: 'STG/m_FIX_EMPTYWF',
        hasRecipe: true,
        workflow: '',
        executionOrder: 1,
      },
      {
        id: 'recipe:_ETL_m_FIX_NOWF.json',
        kind: 'recipe',
        name: '_ETL_m_FIX_NOWF.json',
        layer: 'STG',
        mappingPath: 'STG/m_FIX_NOWF',
        hasRecipe: true,
        executionOrder: 1,
      },
    )
    ...
    expect(ungrouped.tasks.map(t => t.task_id).sort()).toEqual([
      '_ETL_m_FIX_EMPTYWF.json',
      '_ETL_m_FIX_NOWF.json',
    ])
  })
  ...
  it('a recipe with no edges at all gets depends_on: [], lays out at its order-rank column, never throws', () => {
    const rel: RelationshipsT = {
      nodes: [
        {
          id: 'recipe:_ETL_m_SOLO.json',
          kind: 'recipe',
          name: '_ETL_m_SOLO.json',
          layer: 'STG',
          mappingPath: 'STG/m_SOLO',
          hasRecipe: true,
          workflow: 'wf_SOLO',
          executionOrder: 3,
        },
