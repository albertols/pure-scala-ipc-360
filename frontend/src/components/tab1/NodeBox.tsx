...
export const NODE_STYLES: Record<
  string,
  { color: string; bg: string; border: string; abbr: string }
> = {
  source: {
    color: '#34d399',
          ...
          <g key={i} onClick={rowClick} style={rowClick ? { cursor: 'pointer' } : undefined}>
            {isIn && (
              <circle
                ...
                cy={py}
                r={4}
                fill={port.linked ? style.color : '#1e2438'}
                stroke={style.border}
                strokeWidth={1}
              />
            )}
            ...
            {hasExpr && (
              <text
                ...
                textAnchor="middle"
                fill="#818cf8"
                style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 7, fontWeight: 700 }}
              >
                ƒ
              </text>
            )}
            <text
              x={node.x + 12}
              y={py + 4}
              fill={port.linked ? '#c8d3e8' : '#3a4560'}
              style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9.5 }}
            >
              {port.name}
            </text>
            <text
              x={node.x + (hasExpr ? NODE_WIDTH - 26 : NODE_WIDTH - 10)}
              y={py + 4}
              textAnchor="end"
              fill="#3a4560"
            ...
            {wireClick && isIn && (
              <circle
                data-testid={`ipc-port-in-${node.id}-${port.name}`}
                cx={node.x}
                cy={py}
                r={PORT_HIT_R}
                fill="transparent"
            ...
            {wireClick && isOut && (
              <circle
                data-testid={`ipc-port-out-${node.id}-${port.name}`}
                cx={node.x + NODE_WIDTH}
                cy={py}
                r={PORT_HIT_R}
                fill="transparent"
