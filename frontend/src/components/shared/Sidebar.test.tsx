...
const MIXED_FS: FSDir = {
  name: 'xmltobq', layer: 'root', children: [
    {
      name: 'CDM', layer: 'CDM', children: [
        { name: '_ETL_m_FIX.json', path: 'CDM/m_FIX/_ETL_m_FIX.json', type: 'json' },
        { name: 'm_FIX.xml', path: 'CDM/m_FIX/m_FIX.xml', type: 'xml' },
      ],
    },
    {
      name: 'ONLY_XML', layer: 'ODS', children: [
        { name: 'm_ONLY_XML.xml', path: 'ODS/m_ONLY_XML/m_ONLY_XML.xml', type: 'xml' },
      ],
    },
  ],
}
...
describe('Sidebar — opt-in fileFilter (Task 14)', () => {
  it('with a fileFilter keeping only _ETL_*.json, XML entries are absent and an all-XML directory is not rendered', () => {
    render(<Sidebar searchQuery="" selectedPath={null} onSelectFile={() => {}} filesystem={MIXED_FS} fileFilter={etlOnlyFilter} />)
  ...
  it('with no fileFilter, today\'s tree renders unchanged', () => {
    render(<Sidebar searchQuery="" selectedPath={null} onSelectFile={() => {}} filesystem={MIXED_FS} />)
  ...
  it('renders an optional footer after extraContent', () => {
    render(
      <Sidebar searchQuery="" selectedPath={null} onSelectFile={() => {}} filesystem={FS}
        extraContent={<div>extra</div>} footer={<div>footer content</div>} />,
    )
