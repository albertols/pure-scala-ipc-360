...
export function OperationalCard({
  ...
  const edge =
    kp.statusEdge === 'left'
      ? {
          borderLeftWidth: STATUS_EDGE_PX,
        ...
        {runs.length > 0 ? (
          ...
          <div style={{ display: 'flex', gap: 1.5 }}>
            {card.history.map((s, i) => (
              <div
                key={i}
                title={`Run ${card.history.length - i}: ${s}`}
                style={{
                  width: 5,
                  height: 14,
                  borderRadius: 1.5,
      ...
      {card.kind === 'recipe' && card.stats.avg_time_s > 0 && (
        ...
        >
          {[
            {
              label: 'avg',
              val: fmt(card.stats.avg_time_s),
              tip: 'Average execution time across all recorded runs',
            },
            {
              label: 'p50',
              val: fmt(card.stats.p50),
              tip: 'Median execution time (50th percentile)',
            },
            {
              label: 'p95',
              val: fmt(card.stats.p95),
              tip: '95th percentile — 1 in 20 runs takes longer than this',
            },
            {
              label: 'p99',
              val: fmt(card.stats.p99),
              tip: '99th percentile — outlier upper bound',
            },
          ].map(({ label, val, tip }) => (
            <div
              key={label}
          ...
          >
            <span
              style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 3 }}
            >
              avg rows{' '}
              <InfoTooltip text="Average number of rows processed per run" placement="top" />
            </span>
            <span style={{ color: '#c8d3e8', fontFamily: 'JetBrains Mono, monospace' }}>
              {fmtCount(card.stats.avg_count)}
            </span>
          </div>
        </div>
      )}
