...
import {
  layoutLineage, countCrossings, TIER_OF, LINEAGE_FOOTPRINT, DUMMY_HEIGHT,
} from './lineageLayout'
import type { LineageNodeT, LineageT } from '../../api/clusterQueries'

const n = (id: string, hop: number, layer: string, kind: 'table' | 'recipe' = 'table'): LineageNodeT =>
  ({ id, kind, name: id, layer, hop, clusters: [] })
...
describe('tier bands', () => {
  it('groups the layers into the medallion tiers the palette uses', () => {
    ...
    expect(new Set([TIER_OF.STG, TIER_OF.DWH, TIER_OF.CDM, TIER_OF.OUTPUT, TIER_OF.UNKNOWN]).size).toBe(5)
  })
  ...
  it('keeps bronze above silver above gold, in every column', () => {
    ...
    const out = layoutLineage(
      [n('s1', 0, 'STG'), n('d1', 0, 'DWH'), n('c1', 0, 'CDM'),
       n('s2', 1, 'ODS'), n('d2', 1, 'ETL'), n('c2', 1, 'QDM')],
...
describe('long-edge routing', () => {
  ...
  it('treats a gap in the hop numbering as one column, not several', () => {
    const out = layoutLineage(
      [n('a', 0, 'STG'), n('m', 1, 'ODS'), n('z', 9, 'CDM')], [e('a', 'z')])
  ...
  it('routes a long edge through slots that no card occupies', () => {
    ...
    const insideCard = mid.x >= m.x && mid.x <= m.x + LINEAGE_FOOTPRINT.width
      && mid.y >= m.y && mid.y <= m.y + LINEAGE_FOOTPRINT.height
  ...
  it('handles a backward edge without looping forever', () => {
    ...
    const out = layoutLineage(
      [n('a', 0, 'STG'), n('b', 1, 'ODS')], [e('a', 'b'), e('b', 'a')])
...
describe('crossing reduction', () => {
  ...
  const crossed = () => {
    const nodes = [
      n('a1', 0, 'STG'), n('a2', 0, 'STG'), n('a3', 0, 'STG'),
      n('b1', 1, 'STG'), n('b2', 1, 'STG'), n('b3', 1, 'STG'),
    ]
  ...
  it('never overlaps two placed boxes', () => {
    ...
    for (let i = 0; i < real.length; i++) {
      for (let j = i + 1; j < real.length; j++) {
        ...
        const hit = a.x < b.x + LINEAGE_FOOTPRINT.width && b.x < a.x + LINEAGE_FOOTPRINT.width
          && a.y < b.y + LINEAGE_FOOTPRINT.height && b.y < a.y + LINEAGE_FOOTPRINT.height
