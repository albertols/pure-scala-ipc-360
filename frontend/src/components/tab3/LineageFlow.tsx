...
const BAND_INK: Record<string, string> = {
  bronze: '#b0764a', silver: '#9aa6b8', gold: '#d4a537',
  platinum: '#cfd8e6', unresolved: '#4a5570',
}
...
export function LineageFlow({
  ...
  useEffect(() => { setOffsets({}); setSelected(null) }, [nodeId])
  ...
  for (const n of data.nodes) for (const c of n.clusters) clusterCounts.set(c, (clusterCounts.get(c) ?? 0) + 1)
  const clusters = [...clusterCounts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))

  const layers = [...new Set(data.nodes.map(n => n.layer))]
  const selectedNode = selected ? byId.get(selected) : null

  const isDim = (n: LineageNodeT) =>
    !matchesFilter(n) || (traced !== null && !traced.has(n.id))
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', fontSize: 11 }}>
          <span data-testid="lineage-summary" style={{ color: 'var(--text-muted)', fontFamily: 'JetBrains Mono, monospace' }}>
            {`${upstream} upstream · ${downstream} downstream · ${data.nodes.length} nodes`}
          </span>
          {data.truncated && (
            <span data-testid="lineage-truncation" style={{ color: 'var(--yellow)', fontFamily: 'JetBrains Mono, monospace' }}>
              {`⚠ showing ${data.nodes.length} of ${data.totalReachable} — nearest hops complete`}
            </span>
          )}
          {data.truncated && limit < LINEAGE_MAX_LIMIT && (
            <button onClick={() => setLimit(Math.min(LINEAGE_MAX_LIMIT, limit * 2))} style={chipBtn}>expand →</button>
          )}
          {dimmedCount > 0 && (
            <span data-testid="lineage-filter-note" style={{ color: 'var(--text-dim)', fontFamily: 'JetBrains Mono, monospace' }}>
              {`${dimmedCount} dimmed`}
            </span>
          )}
          {Object.keys(offsets).length > 0 && (
            <button onClick={() => setOffsets({})} style={chipBtn}>reset layout</button>
          )}
          <div style={{ flex: 1 }} />
        ...
        {clusters.length > 0 && (
          <div data-testid="lineage-clusters" style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <MultiFilterChips testId="lineage-layer-filter" label="Layer" options={layers}
            selected={layerFilter} onToggle={setLayerFilter}
            colors={Object.fromEntries(layers.map(l => [l, layerColor(l)]))} />
          <MultiFilterChips testId="lineage-status-filter" label="Status" options={['OK', 'KO', 'PENDING']}
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, gap: 10 }}>
        <div ref={scrollRef} data-testid="lineage-scroll" style={{ flex: 1, overflow: 'auto', minHeight: 0, position: 'relative' }}>
          <div style={{ position: 'relative', width: layout.width + RAIL_W, height: layout.height }}>

            {/* tier rails — sticky so the band a node sits in stays named while scrolling */}
            {layout.bands.map(b => (
              <div key={b.tier} data-testid="lineage-band" style={{
                position: 'absolute', left: 0, top: b.y - 8,
                width: layout.width + RAIL_W, height: b.height + 16,
            ...
            <svg width={layout.width + RAIL_W} height={layout.height}
              ...
              <defs>
                ...
                <marker id="lin-arrow-hot" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
            ...
            {layout.nodes.filter(p => !p.isDummy).map(p => {
              const n = p.node!
              const pos = at(p)
              ...
              return (
                <div
                  ...
                  style={{
                    position: 'absolute', left: pos.x + RAIL_W, top: pos.y,
                    width: LINEAGE_FOOTPRINT.width,
                    cursor: 'grab',
                    opacity: dim ? 0.28 : 1,
                    transition: 'opacity 0.12s',
                    outline: isSeed ? `2px solid ${kindPalette(n.kind).accent}`
                      : selected === n.id ? '2px solid var(--blue)' : 'none',
            ...
            {[...new Map(layout.nodes.filter(p => !p.isDummy)
              .map(p => [p.node!.hop, p])).entries()]
              .sort((a, b) => a[0] - b[0])
              .map(([hop, p]) => (
                <div key={hop} style={{
                  position: 'absolute', left: p.x + RAIL_W, top: layout.height - 4,
                  width: LINEAGE_FOOTPRINT.width, textAlign: 'center',
        ...
        {selectedNode && (
          <div data-testid="lineage-details" style={{
            width: 264, flexShrink: 0, overflow: 'auto',
            borderLeft: '1px solid var(--border)', paddingLeft: 10,
            display: 'flex', flexDirection: 'column', gap: 10,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text)', flex: 1 }}>Details</span>
              <button aria-label="Close details" onClick={() => setSelected(null)}
                style={{ background: 'none', border: 'none', color: '#4a5570', cursor: 'pointer', fontSize: 12 }}>✕</button>
            </div>
            <OperationalCard card={toCard(selectedNode, statusById[selectedNode.id] ?? 'PENDING')} selected />
            <button
              aria-label="Center lineage here"
              onClick={() => onReseed?.(selectedNode.id)}
              style={{ ...chipBtn, width: '100%', padding: '5px 8px' }}
            >⌖ center lineage here</button>
