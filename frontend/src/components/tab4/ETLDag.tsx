        ...
        {hasSubDag && (
          ...
          >
            <svg width="9" height="9" viewBox="0 0 9 9" fill="none">
              <path
                d={subExpanded ? 'M1 2.5l3.5 4 3.5-4' : 'M2.5 1l4 3.5-4 3.5'}
                stroke="#4a5570"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
            ...
            <div key={dag.dag_id}>
              ...
              >
                <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                  <path
                    d={dagExpanded ? 'M1 3l4 4 4-4' : 'M3 1l4 4-4 4'}
                    stroke="#4a5570"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                  />
...
function TaskNode({
  ...
  return (
    <g onClick={onClick} style={{ cursor: 'pointer' }}>
      ...
      />
      <rect x={task.x ?? 0} y={task.y ?? 0} width={5} height={TASK_H} rx={3} fill={color} />
      <text
        x={(task.x ?? 0) + 14}
        y={(task.y ?? 0) + 18}
        fill="#e2e8f8"
      ...
      >
        {task.last_status}
        {task.duration_s > 0 ? ` · ${task.duration_s}s` : ''}
      </text>
      {isSubDag && (
        <>
          <rect
            x={(task.x ?? 0) + TASK_W - 26}
            y={(task.y ?? 0) + 8}
            width={18}
            height={14}
            rx={3}
          ...
          />
          <text
            x={(task.x ?? 0) + TASK_W - 17}
            y={(task.y ?? 0) + 18}
            textAnchor="middle"
            fill="#4f9cf9"
            style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 8, fontWeight: 700 }}
          >
            SD
          </text>
        </>
...
function DagCanvas({
  ...
  return (
    ...
    >
        ...
        {edges.map((e, i) => {
          ...
          return (
            <path
              ...
              fill="none"
              stroke={e.hi ? '#4f9cf9' : '#2a3050'}
              strokeWidth={e.hi ? 1.8 : 1.2}
      ...
      >
        {[
          { i: '+', f: () => setZoom(z => Math.min(2.5, z + 0.2)) },
          { i: '−', f: () => setZoom(z => Math.max(0.3, z - 0.2)) },
          {
            i: '⊡',
            f: () => {
              setZoom(1)
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '0 0 8px', fontSize: 12, color: '#7b88aa', lineHeight: 1.6 }}>
          This will publish a replay message to the GCP Pub/Sub topic{' '}
          <code style={{ color: '#4f9cf9', fontSize: 11 }}>etl-replay-trigger</code>.
        </p>
        <div
          style={{
        ...
        >
          ...
          <pre style={{ margin: 0, color: '#c8d3e8', lineHeight: 1.6 }}>
            {JSON.stringify(
              {
                dag_id: dagId,
                task_id: taskId,
                replay_from: taskId,
                triggered_by: 'etl360-ui',
  ...
  return (
    <div>
      ...
      >
        Run History{' '}
        <InfoTooltip
          text="Last executions of this DAG. Click any run to view details."
          placement="right"
        />
      ...
      <div style={{ display: 'flex', gap: 3, marginBottom: 8 }}>
        {runs.map(r => (
          <div
            key={r.run_id}
            title={`${r.run_id} — ${r.status} (${r.duration_s}s)`}
            onClick={() => onSelectRun(r.run_id)}
            style={{
              width: 18,
              height: 28,
              borderRadius: 3,
              background: color[r.status] ?? '#4a5570',
              cursor: 'pointer',
              opacity: 0.85,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {[...runs].reverse().map(r => (
          ...
          >
            ...
            <span style={{ color: '#4a5570', fontFamily: 'JetBrains Mono, monospace' }}>
              {r.duration_s}s
            </span>
          </div>
        ))}
...
export function ETLDag() {
  ...
  const litDag = useMemo(() => (dag ? overlayRun(dag, rowsForDate) : null), [dag, rowsForDate])
  const litClusters = useMemo(
    () => clusters.map(c => overlayRun(c, rowsForDate)),
            ...
            {selectedTask && (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: 2,
                      background: STATUS_COLOR[selectedTask.last_status] ?? '#4a5570',
                    }}
                  />
                  <span
                    style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {(
                    [
                      ...
                      [
                        'Duration',
                        selectedTask.duration_s > 0 ? `${selectedTask.duration_s}s` : '—',
                      ],
                    ...
                    <div key={k} style={{ display: 'flex', gap: 8 }}>
                      <span style={{ fontSize: 10, color: '#4a5570', width: 68, flexShrink: 0 }}>
                        {k}
                      </span>
                      <span
                        style={{
                ...
                {selectedDagId && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    >
                      Replay Controls
                      <InfoTooltip
                        text="Publishes a Pub/Sub message to the etl-replay-trigger topic. The Airflow DAG listener will re-queue the selected branch."
                        placement="right"
                      />
                    </div>
                    <button
                      onClick={() =>
                        setReplayModal({ dagId: selectedDagId, taskId: selectedTask.task_id })
                      }
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    <button
                      onClick={() =>
                        setReplayModal({
                          dagId: selectedDagId,
                          taskId: selectedTask.task_id + '-branch',
                        })
                      }
                      style={{
                        display: 'flex',
                ...
                {selectedTask.sub_dag && (
                  <div>
                    <div style={{ fontSize: 10, color: '#4a5570', marginBottom: 6 }}>
                      Sub-DAG: {selectedTask.sub_dag.dag_id}
                    </div>
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: 6,
                              borderRadius: '50%',
                              background: STATUS_COLOR[st.last_status],
                            }}
                          />
                          <span
                            style={{ fontFamily: 'JetBrains Mono, monospace', color: '#7b88aa' }}
                          >
                            {st.task_id}
                          </span>
                          <span
