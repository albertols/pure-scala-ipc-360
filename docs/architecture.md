...
| `GET /api/operational/lineage` | One node's transitive upstream+downstream closure (`docs/adr/0020-lineage-flow.md`): `?node=` (unknown → 404), `?limit=` (default 150, max 600, over-range → 400). Each node carries a **signed hop** — negative upstream, `0` seed, positive downstream — computed during the traversal that knows the direction, so the view never re-derives it from edges. Breadth-first, so the budget cuts the FURTHEST hops and the nearest lineage is complete by construction; `truncated` + `totalReachable` state how much was dropped. Cycle-safe. Deliberately **not** cluster-scoped — lineage crosses cluster boundaries, and stopping at the selection would draw a complete-looking flow that is not one |
