...
| `GET /api/operational/summary` | Cross-date rollup (`OperationalSummaryDto { dates, recipes[] }`): per-recipe `layer` (`UNKNOWN` if absent from L2L), 14-entry `history`, `okCount`/`koCount`, nearest-rank `avg`/`p50`/`p95DurationMin`, `lastJobId`/`lastClusterName` — computed in `OperationalService`, joined to `LayerToLayerService` by `recipe_filename`. Optional repeatable `?clusters=` narrows the **recipe** set to those that ran in the named b15 clusters (resolved through `ClusterIndexService`, one `index()` call per request); the **date axis is never narrowed**, and omitted or empty the response is byte-identical to the unscoped rollup — the payload is O(recipes × dates), so an unscoped request at ~7 000 recipes is tens of megabytes |
...
| `GET /api/operational/lineage` | One node's transitive upstream+downstream closure (`docs/adr/0020-lineage-flow.md`): `?node=` (unknown → 404), `?limit=` (default 150, max 600, over-range → 400), `?cluster=`, `?prefer=`. Each node carries a **signed hop** — negative upstream, `0` seed, positive downstream — computed during the traversal that knows the direction, so the view never re-derives it from edges. Breadth-first, so the budget cuts the FURTHEST hops and the nearest lineage is complete by construction; `truncated` + `totalReachable` state how much was dropped. Cycle-safe. Unscoped by default (byte-identical to before); `?cluster=` (a name, or `auto` to server-resolve from `?prefer=`) narrows to `{seed} + recipes(cluster) + adjacent tables`, with out-of-cluster recipes returned as named `gateway` stubs rather than silently absent (`docs/adr/0021-lineage-cluster-scope.md`) |
...
- `docs/adr/0001`–`0020` — the decisions behind this shape, with rejected alternatives.
