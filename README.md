...
`prettier` (pinned 3.9.6) formats `frontend/src/**/*.{ts,tsx}` and the root `*.ts`
configs. `pnpm format` writes, `pnpm format:check` verifies, and `make check` runs the
latter **unguarded** — a formatting violation fails the target, same as a type error.

The config (`frontend/.prettierrc`) matches the style the codebase was already written
in — `singleQuote`, `semi: false`, `printWidth: 100`, `arrowParens: "avoid"` — rather
than imposing a new one.

This replaced `oxfmt`, which was wired up but never run: it accepts **no configuration
at all** (no quote style, no print width), and 142 of 143 files failed it because the
codebase simply isn't in its style. Adopting it would have flipped ~11,800 single-quoted
literals to double and grown the frontend ~42%. Prettier reaches the same "checked
formatting" goal at ~25% growth while keeping the existing quote style, and is a mature
tool whose defaults won't shift under a 0.x release.

Deliberately **not** formatted, each for a reason (see `frontend/.prettierignore`):
`*.md` (docs are hand-wrapped at ~90 chars and use wide tables prettier reflows onto
single enormous lines), `src/api/__fixtures__/*.json` (captured API responses that must
stay byte-identical to backend output), `index.css` (the ADR-0005 token file), and
`src/api/types.gen.ts` (generated by `make generate-api`).
